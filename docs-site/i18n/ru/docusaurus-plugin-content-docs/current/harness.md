# Среда запуска Unified Harness

{/* Release-version snippets in this file are generated by scripts/release.py. */}

:::note[Стабильный релиз 0.9]

Линейка `gigaloom` 0.9 добавляет work-first surface, ограниченный Thread Relay
и точные route overlays для `gpt2giga 0.3`. Совместимость с provider CLI и явно
помеченные preview API могут развиваться. Используйте документированные
локальные и контролируемые границы, а не unattended multi-user сервис.
...
## Поверхности work-first и gateway в 0.9

Основная browser composition —
`Project -> Thread -> Run -> Evidence -> Action`. Work показывает перед
отправкой единую сводку route/model support, workspace, read-only Effective
Instructions, authority и blockers; Inbox, Automations, Library и More
показывают существующих владельцев без дублирования state.

Пять основных CLI-примеров:
...
Gateway selectors расположены до agent token и разрешают один immutable route
без fallback. Dry-run Thread Relay читает bounded target projection и не
выполняет mutation или provider call. Экспорт schemas и evidence — явные
локальные операции. Полные контракты приведены в
[Work, потоках и контексте](work-threads-and-context.md) и
[интеграции с gateway](gateway-integration.md).
...
| Pipe/stdin | `printf 'task' \| giga claude -p -` | stdin и необработанные байты передаются без изменений. |
...
| CI | `CI=1 giga gemini -p task` | CLI работает напрямую, без формирования запроса в Harness. |
...
Текущий дистрибутив `gigaloom==0.9.1` добавляет только команду `giga`; его
...
[контракт контролируемого сетевого доступа](architecture/network-access.md).
...
описаны в [правилах идентификации удалённых пользователей](architecture/remote-user-identity.md). Не
...
Текущая metadata `gigaloom==0.9.1` сохраняет
