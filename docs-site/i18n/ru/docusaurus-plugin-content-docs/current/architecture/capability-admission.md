...
Реестр возможностей и пользовательский интерфейс используют единые термины из
этого контракта и не вводят собственные варианты. Решение не меняет
существующие машинные форматы, не переносит сессии, не выбирает маршрут
провайдера, не выдаёт полномочия инструментам и не изменяет подсистему
Automation. Для каждого такого изменения требуется отдельная реализация и
проверка.
