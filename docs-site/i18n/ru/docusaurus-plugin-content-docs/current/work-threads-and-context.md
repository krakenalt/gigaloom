...
При создании нового чата Codex CLI GigaLoom регистрирует для агента инструменты
`thread.list`, `thread.read`, `thread.send` и `thread.status`. Агент может сам
найти другой чат того же проекта и адресовать ему сообщение: пользователю не
нужно добавлять `@`-упоминание или искать отдельную кнопку отправки. Уже
созданные upstream-чаты не получают динамические инструменты задним числом —
для этого нужно создать новый чат или ответвление.

Первый вызов `thread.send` всегда создаёт безопасный предпросмотр без текста
сообщения. Доставка выполняется только после того, как пользователь подтвердит
в Approval Center именно этого отправителя, проект, ревизию цели и предпросмотр.
Разрешение используется один раз, а в квитанции остаются только хэши и состояние.
Компактный список `@` теперь служит лишь для добавления в текущий черновик
контекста, возможностей и файлов репозитория.
...
## Сжатие контекста Codex

На экране Work введите `/compact` как единственный текст черновика или выберите
**+ -> Сжать контекст**. GigaLoom вызывает нативное сжатие Codex app-server для
точного неактивного и уже загруженного чата; команда `/compact` не отправляется
модели как запрос. Нужны работающий владелец app-server и актуальная ревизия
запуска. В ответе сохраняются только безопасные upstream-идентификаторы.
