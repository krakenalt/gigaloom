...
const tabs: readonly OperationalTab[] = [
  { id: "arena", labelKey: "arena", href: "/web/evaluation/arena" },
  { id: "evals", labelKey: "evals", href: "/web/evaluation/evals" },
  { id: "baselines", labelKey: "baselines", href: "/web/evaluation/baselines" },
];
    ...
    <>
      ...
      {rows.length === 0 ? <div className="empty-state">{message(locale, section === "evals" ? "noEvaluationResults" : "noItems")}</div> : (
        <div className="operations-table" role="table">
          {section === "arena" ? query.data.arenas.map((item) => <OperationalRowLink selected={selectedId === item.id} selectedId={item.id} to="/web/evaluation/arena" key={item.id}><div><strong>{item.id}</strong><span>{item.harnessCount} {message(locale, "harnesses")}</span></div><span>{item.createdAt}</span><span>{item.updatedAt}</span><StatusBadge status={item.status} /></OperationalRowLink>) : null}
          {section === "evals" ? query.data.evals.map((item) => <OperationalRowLink selected={selectedId === item.name} selectedId={item.name} to="/web/evaluation/evals" key={item.name}><div><strong>{item.name}</strong><span>{item.description}</span></div><span>{item.caseCount} {message(locale, "cases")}</span><span>{item.latestScore ?? "—"}</span><StatusBadge status={item.latestStatus ?? "not run"} /></OperationalRowLink>) : null}
          {section === "baselines" ? query.data.baselines.map((item) => <OperationalRowLink selected={selectedId === item.specName} selectedId={item.specName} to="/web/evaluation/baselines" key={item.specName}><div><strong>{item.specName}</strong><span>{item.evalRunId}</span></div><span>{item.pinnedAt ?? "—"}</span><span>{message(locale, "reviewedIntent")}</span><StatusBadge status="pinned" /></OperationalRowLink>) : null}
        </div>
      )}
    </>
  );
