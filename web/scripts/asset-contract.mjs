...
export async function frontendInputFiles() {
  const files = [
    ...
    join(frontendRoot, "branding", "generate-assets.mjs"),
    canonicalBrandPath,
  ];
  return files.sort((left, right) => compareNames(
    normalizePath(relative(repositoryRoot, left)),
    normalizePath(relative(repositoryRoot, right)),
  ));
}

export async function namedFilesDigest(files, root = repositoryRoot) {
