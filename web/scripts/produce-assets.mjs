...
import { frontendRoot, outputRoot, repositoryRoot } from "./asset-contract.mjs";
...
const temporary = await mkdtemp(join(parent, ".web-assets-build-"));
const backup = `${outputRoot}.previous-${process.pid}`;
const buildEnvironment = {
  GIGALOOM_WEB_OUTPUT: temporary,
...
try {
  run(
    process.execPath,
    [join(frontendRoot, "branding", "generate-assets.mjs"), "--frontend-only"],
  );
  run(
    process.execPath,
