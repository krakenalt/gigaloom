...
:::note[Stable 0.9 release]

The `gigaloom` 0.9 line adds the work-first surface, bounded Thread Relay, and
exact `gpt2giga 0.3` route overlays. Provider CLI compatibility and explicitly
marked preview APIs can still evolve; use the documented local and supervised
boundaries rather than treating GigaLoom as an unattended multi-user service.
...
## 0.9 work-first and gateway surfaces

The primary browser composition is
`Project -> Thread -> Run -> Evidence -> Action`. Work presents one pre-submit
summary of route/model support, workspace, read-only Effective Instructions,
authority, and blockers; Inbox, Automations, Library, and More expose their
existing owners without duplicating state.

The five core CLI examples are:
...
Gateway selectors appear before the agent token and resolve one immutable route
without fallback. The Thread Relay dry-run reads a bounded target projection
and performs no mutation or provider call. Schema and evidence exports are
explicit local reads/writes. See
[Work, threads, and context](work-threads-and-context.md) and
[Gateway integration](gateway-integration.md) for the full contracts.
