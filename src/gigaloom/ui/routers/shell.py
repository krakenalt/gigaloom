...
from gigaloom.ui.routers.schemas import (
...
def create_shell_router(security: HarnessUISecurity) -> APIRouter:
    ...
    async def begin_remote_login(request: Request, next: str = "/web/work"):
        if security.local_mode:
    ...
    async def recover_local_browser_session(request: Request) -> Response:
        ...
        response = RedirectResponse("/web/settings", status_code=303)
        security.set_session_cookie(response, session)
        return response
    ...
    @router.fs_read.get("/web/assets/{asset_name:path}", include_in_schema=False)
    def web_asset(asset_name: str, request: Request) -> Response:
        encoding = _accepted_encoding(request.headers.get("accept-encoding"))
        try:
            content, asset = load_web_asset(asset_name, encoding=encoding)
    ...
    @router.fs_read.get("/web", response_class=HTMLResponse, include_in_schema=False)
    @router.fs_read.get(
        "/web/{spa_path:path}",
        response_class=HTMLResponse,
        include_in_schema=False,
    )
    def web_shell(spa_path: str = "") -> HTMLResponse:
        normalized = spa_path.strip("/")
        if normalized and _COCKPIT_V2_PATH.fullmatch(normalized) is None:
            raise HTTPException(status_code=404, detail="Not found")
        try:
            content = load_web_shell()
        except CockpitV2UnavailableError as exc:
            raise _cockpit_unavailable(exc) from exc
